...
export default function RegisterPage({ initialEmail = "" }: RegisterPageProps) {
  ...
  const [errors, setErrors] = useState<
    Partial<Record<keyof PasswordRegisterFormValues, string>>
  >({});
  ...
  const handleChange = (
    key: keyof PasswordRegisterFormValues,
    value: string,
  ) => {
    setValues((prev: PasswordRegisterFormValues) => ({
      ...prev,
      [key]: value,
    }));
  ...
  const parseErrors = (
    result: ReturnType<typeof registerWithPasswordSchema.safeParse>,
  ) => {
    if (result.success) return {};
    const fieldErrors: Partial<
      Record<keyof PasswordRegisterFormValues, string>
    > = {};
    for (const issue of result.error.issues) {
      const field = issue.path[0] as
        | keyof PasswordRegisterFormValues
        | undefined;
          ...
          <div className="flex flex-col items-center space-y-6">
            <div className="text-center">
              ...
              <p className="text-xs text-slate-500 mt-2">
                Use email and password to create your Overflow identity.
              </p>
            </div>
