...
class AuthService {
	...
	async loginWithPassword(
		...
		const response = await fetch(
			getApiUrl('/api/public/auth/password/login'),
			{
				method: 'POST',
	...
	async registerWithPassword(
		...
		const response = await fetch(
			getApiUrl('/api/public/auth/password/register'),
			{
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				credentials: 'include',
	...
	private async performRefresh(): Promise<AuthResponse> {
		...
		const response = await fetch(getApiUrl('/api/public/auth/refresh'), {
			method: 'POST',
			credentials: 'include',
			cache: 'no-store',
		});
	...
	async reloadIntraData(userId: number): Promise<User> {
		...
		const response = await this.authenticatedFetch(
			getApiUrl(`/api/auth/reload?userId=${userId}`),
			{
				method: 'POST',
			}
		);
	...
	async logoutSession(sessionId: string): Promise<void> {
		...
		const response = await this.authenticatedFetch(
			getApiUrl(`/api/auth/logout${query}`),
			{
				method: 'POST',
			}
		);
	...
	async adminUpdateUser(
		...
		this.invalidateUserCache(userId);
		const response = await this.authenticatedFetch(
			getApiUrl(`/api/auth/admin/users/${userId}`),
			{
				method: 'PATCH',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify(payload),
	...
	async adminLogoutUser(userId: number): Promise<void> {
		const response = await this.authenticatedFetch(
			getApiUrl(`/api/auth/admin/users/${userId}/logout`),
			{
				method: 'POST',
			}
		);
	...
	async adminCreateUser(payload: CreateUserPayload): Promise<User> {
		this.invalidateUserCache();
		const response = await this.authenticatedFetch(
			getApiUrl('/api/auth/admin/users/create'),
			{
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify(payload),
	...
	async adminDeleteUser(userId: number): Promise<void> {
		this.invalidateUserCache(userId);
		const response = await this.authenticatedFetch(
			getApiUrl(`/api/auth/admin/users/${userId}/delete`),
			{
				method: 'DELETE',
			}
		);
	...
	private parseExpiresInSeconds(
		expiresIn: AuthResponse['expiresIn']
	): number {
		if (typeof expiresIn === 'number') {
			if (expiresIn > 1_000_000_000_000) {
				return Math.max(
					Math.floor((expiresIn - Date.now()) / 1000),
					10
				);
			}
