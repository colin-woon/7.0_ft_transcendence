...
export function useUserProfile(
  ...
  const hasInitialProfile =
    options?.initialProfile !== null && options?.initialProfile !== undefined;
  const hasTerminalInitialError =
    options?.initialErrorStatus === 403 || options?.initialErrorStatus === 404;
  const skipInitialFetchRef = useRef(
    hasInitialProfile || hasTerminalInitialError,
  );
  ...
  const [errorStatus, setErrorStatus] = useState<number | null>(
    options?.initialErrorStatus ?? null,
  );
    ...
    async (payload: UserUpdatePayload) => {
      ...
      try {
        if (userId && authUser?.id !== userId) {
          throw new Error(
            "Only your own profile can be updated from this page",
          );
        }
