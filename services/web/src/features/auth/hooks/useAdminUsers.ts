...
import type { AdminUpdatePayload, CreateUserPayload, User } from '@/features/auth/api/authService'

export function useAdminUsers() {
  const { adminUpdateUser, adminLogoutUser, adminCreateUser, adminDeleteUser } = useAuth()
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)

  const wrap = useCallback(async <T,>(action: () => Promise<T>): Promise<T | null> => {
    setLoading(true)
    setError(null)
    try {
      return await action()
  ...
  const updateUser = useCallback(
    async (userId: number, payload: AdminUpdatePayload): Promise<User | null> => {
