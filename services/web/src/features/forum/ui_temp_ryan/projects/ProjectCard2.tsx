...
export function ProjectCard({ project }: ProjectCardProps) {
  ...
  return (
    <div className="group relative block w-full max-w-[200px] aspect-square overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transform-gpu transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-1.5 hover:scale-[1.015] hover:border-slate-400 hover:shadow-2xl">
      {/* Default state */}
      ...
      <div className="pointer-events-none absolute inset-x-2 bottom-2 z-20 flex items-end justify-between transition-all duration-300 group-hover:opacity-0 group-hover:translate-y-1">
        <div
          className={`rounded-[7px] border px-2 py-1 text-[9px] font-semibold uppercase tracking-[0.06em] ${difficultyTone}`}
        >
          {project.difficulty}
        </div>
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-6 z-20 flex items-center justify-center opacity-0 transition-all duration-300 delay-100 group-hover:opacity-100 group-hover:pointer-events-auto">
        <Link
          ...
          className="flex items-center gap-1.5 rounded-full border border-slate-300 bg-transparent px-4 py-1.5 text-[10px] font-bold uppercase tracking-wider text-slate-600 transition-colors duration-300 hover:bg-slate-900 hover:text-white active:scale-95"
        >
          View Project
          <svg
            className="h-3 w-3"
