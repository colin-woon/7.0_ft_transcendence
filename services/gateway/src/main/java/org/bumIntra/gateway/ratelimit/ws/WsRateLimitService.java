...
public class WsRateLimitService implements WsRateLimiter {
     ...
     */
    @Override
    public Uni<Boolean> allowPreAuthConnection(String clientIp) {
        String key = "ws:conn:" + clientIp;
        return allow(key, WsRateLimitProfiles.WS_PREAUTH_CONN);
    }
     ...
     */
    @Override
    public Uni<Boolean> allowAuthenticatedConnection(String userId) {
        String key = "ws:conn:user:" + userId;
        return allow(key, WsRateLimitProfiles.WS_AUTH_CONN);
    }

    /**
     * Key format: ws:frame:{userId}
     */
    @Override
    public Uni<Boolean> allowFrameTraffic(String userId) {
        String key = "ws:frame:" + userId;
        return allow(key, WsRateLimitProfiles.WS_FRAME);
    }
