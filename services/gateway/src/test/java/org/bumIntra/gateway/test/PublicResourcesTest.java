...
public class PublicResourcesTest {
	...
	public void testGetSuccessLogic() {
		when(auth.proxyGet("auth/login/test"))
				.thenReturn(Response.ok("logged in").build());

		given()
				.when().get("/api/public/auth/login/test")
				.then()
				.statusCode(200)
	...
	public void testGetSuccessCallback() {
		...
		given()
				.when().get("/api/public/auth/callback/test")
				.then()
				.statusCode(200)
	...
	public void testGetFailure() {
		given()
				.when().get("/api/public/auth/unknown/test")
				.then()
	...
	public void testGetTraversalLikePathBlocked() {
		given()
				.when().get("/api/public/auth/login/../me")
				.then()
				.statusCode(anyOf(equalTo(404), equalTo(405)));
	...
	public void testPostSuccess() {
		when(auth.proxyPost("auth/refresh", new byte[0]))
				.thenReturn(Response.ok("refreshed").build());

		given()
				.when().post("/api/public/auth/refresh")
				.then()
				.statusCode(200)
	...
	public void testPostFailure() {
		given()
				.when().post("/api/public/auth/refresh/me")
				.then()
				.statusCode(anyOf(equalTo(404), equalTo(405)));
