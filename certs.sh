...
trap abort_interrupt INT TERM

prompt_secret() {
    local var_name="$1"
    local value

    printf "%s is not set in shared env. (CTRL+C to quit)\n" "$var_name"
    printf "Enter %s password: " "$var_name"
    stty -echo
    ...
    if [ -z "$value" ]; then
        echo "$var_name is required. Aborted."
        exit 1
    fi
...
if [ -f "./environment/shared.env" ]; then
    set -a
...
if [ -z "$KEYSTORE_PASS" ]; then
    prompt_secret KEYSTORE_PASS
fi

if [ -z "$TRUSTSTORE_PASS" ]; then
    prompt_secret TRUSTSTORE_PASS
fi
